...
export async function generateMetadata({ searchParams }) {
  try {
    ...
    return {
      ...
      robots: 'index, follow',
      alternates: {
        canonical: `https://xynapseai.net/treemap?chain=${encodeURIComponent(validChain)}${address !== 'unknown' ? `&address=${encodeURIComponent(address)}` : ''}`,
      },
      openGraph: {
        title,
        description,
        url: `https://xynapseai.net/treemap?chain=${encodeURIComponent(validChain)}${address !== 'unknown' ? `&address=${encodeURIComponent(address)}` : ''}`,
        type: 'website',
        images: [
          {
