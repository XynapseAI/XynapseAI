...
async function checkRateLimit(ip, userId) {
  ...
  try {
    ...
    const ipMax = process.env.NODE_ENV === 'development' ? 1000 : 100;
    const userMax = process.env.NODE_ENV === 'development' ? 500 : 50;
...
export async function POST(request) {
  ...
  try {
    ...
    // Enforce reCAPTCHA for POST mutation
    const recaptchaToken = request.headers.get('X-Recaptcha-Token');
    if (!recaptchaToken && process.env.NODE_ENV !== 'development') {
      newCsrfToken = newCsrfToken || await setCSRFToken(ip, userId);
    ...
    if (process.env.NODE_ENV !== 'development') {
      try {
        const recaptchaResponse = await verifyRecaptcha(recaptchaToken, 'post_connect_data', ip);
