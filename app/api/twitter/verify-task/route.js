...
async function trackViolation(ip, reason = 'Unknown') {
  ...
  if (
    [
      ...
      'Invalid JSON body',
      'Invalid input data',
      'Access denied: Invalid user ID',
      'Missing reCAPTCHA token',
    ].includes(reason)
  ) {
    logger.warn(
      `Non-critical violation ignored: ${ip}, reason: ${reason}, violations: ${violations}`,
    )
    return
  }
...
async function withRetry(fn, retries = 2, delay = 1000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      logger.warn(`Operation failed, retrying after ${delay}ms`, {
        attempt: i + 1,
        error: err.message,
      })
...
export async function POST(request) {
  ...
  const corsHeaders = {
    ...
    ...(origin &&
      origin !== 'null' &&
      isAllowedOrigin(origin, referer, pathname) && {
  ...
  const { taskId, userId, recaptchaToken, tweetUrl } = parsedBody
  ...
  if (process.env.NODE_ENV !== 'development') {
    try {
      logger.info('Attempting reCAPTCHA verification', {
  ...
  try {
    ...
    const twitterHandle = await withRetry(
      async () => await prisma.twitter_handles.findUnique({ where: { user_id: userId } }),
    )
    if (!twitterHandle && task.task_type !== 'daily_checkin') {
      ...
      return NextResponse.json(
        { detail: 'X (Twitter) account not connected' },
        { status: 400, headers: corsHeaders },
      )
    }
    ...
    if (
      !task.is_daily &&
      existingCompletion &&
      existingCompletion.completion_count >= task.max_completions
    ) {
      logger.info(`Task ${taskId} max completions reached for user ${userId}`, { ip })
      return NextResponse.json(
        { detail: 'Maximum completions reached' },
        { status: 400, headers: corsHeaders },
      )
    }
    ...
    if (task.task_type === 'follow') {
      isTaskValid = true
      logger.info(
        `Simulated follow task verification: userId=${userId}, targetId=${task.target_id}, isFollowing=${isTaskValid}`,
        { ip },
      )
      ...
      if (
        !tweetUrl ||
        !tweetUrl.trim() ||
        !tweetUrl.match(/^https?:\/\/(www\.)?(x|twitter)\.com\/[a-zA-Z0-9_]+\/status\/\d+/i)
      ) {
        logger.warn('Invalid or missing tweet URL', { ip })
        isTaskValid = false
      } else {
        isTaskValid = true
      }
      ...
      isTaskValid = true
      logger.info(
        `Simulated daily check-in task verification: userId=${userId}, isValid=${isTaskValid}`,
        { ip },
      )
    }
    ...
    let points = task.task_type === 'follow' || task.task_type === 'tweet' ? 20 : 10
    ...
    return NextResponse.json(
      {
        success: true,
        completionCount: existingCompletion ? existingCompletion.completion_count + 1 : 1,
        pointsEarned: points,
      },
      { headers: corsHeaders },
    )
    ...
    return NextResponse.json(
      { detail: `Error verifying task: ${error.message}` },
