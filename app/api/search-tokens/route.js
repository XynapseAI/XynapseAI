...
async function checkRateLimit(ip, endpoint) {
    ...
    try {
        ...
        if (client) {
            await client
                .quit()
                .catch((err) => logger.warn('Redis disconnect failed', { err: err?.message }))
        }
    }
}
...
export async function GET(request) {
    ...
    const referer = request.headers.get('referer')
    const pathname = new URL(request.url).pathname
    logger.info(`GET request to /api/search-tokens from IP ${ip}`, {
        origin,
        referer,
    ...
    if (!(await isAllowedOrigin(origin, referer, pathname))) {
        ...
        logger.error(
            `CORS error: Origin ${origin || 'null'} or Referer ${referer || 'null'} not allowed`,
        )
        return NextResponse.json(
    ...
    try {
        ...
        return NextResponse.json({ success: false, error: err.message }, { status: 403, headers })
    }
    ...
    `
        ...
        const results = await withRetry(() =>
            query(searchSql, [searchTerm, exactMatch, startMatch]),
        )
        ...
        if (processedData.length === 0) {
            try {
                const cgUrl = `https://api.coingecko.com/api/v3/search?query=${encodeURIComponent(searchQuery.trim())}`
                const cgRes = await fetch(cgUrl)
                if (cgRes.ok) {
                    const cgJson = await cgRes.json()
                    const coins = cgJson.coins || []
                    const cgTokens = coins.slice(0, 20).map((coin) => ({
                        coingecko_id: coin.id,
                        symbol: coin.symbol.toUpperCase(),
                        name: coin.name,
                        image: coin.thumb || '/icons/default-token.webp',
                    ...
                    processedData = cgTokens
                    isFallback = true
                    logger.info('Token search fallback to Coingecko', {
                        query: searchQuery,
        ...
        logger.info('Token search completed', {
            ...
            resultCount: processedData.length,
            cacheKey,
            source: isFallback ? 'coingecko' : 'database',
        })
        ...
        await trackViolation(
            ip,
            `Error in token search: ${error.message}`,
            isSystemError ? 'warn' : 'severe',
        )
