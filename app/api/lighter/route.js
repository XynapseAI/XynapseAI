...
const CACHE_KEY = 'lighter:metaAndAssetCtxs'

// === NEW: Function lấy global data (cache + external fetch) ===
async function getGlobalLighterData() {
  const redis = await getRedisClient()
  ...
  if (![orderBooksRes, statsRes, detailsRes, fundingRes].every((r) => r.ok)) {
    throw new Error('Failed to fetch Lighter global data from external API')
  }
...
async function handler(request) {
  ...
  try {
    // Global data (no type)
    if (!type) {
      const data = await getGlobalLighterData()
      return NextResponse.json(data)
    }

    // Các type cần global data → gọi thẳng function (không recursive)
    let marketIdToSymbol = {}
    let orderBooks = []
    if (['user', 'l2book', 'allMids', 'recentTrades', 'openOrders'].includes(type)) {
      const globalData = await getGlobalLighterData()
    ...
    if (type === 'allMids') {
      ...
      orderBooks.forEach((ob) => {
        const bestBid = ob.bids?.[0] ? parseFloat(ob.bids[0][0]) : 0
        const bestAsk = ob.asks?.[0] ? parseFloat(ob.asks[0][0]) : 0
        mids[ob.symbol] = bestBid && bestAsk ? (bestBid + bestAsk) / 2 : 0
      })
    ...
    if (type === 'user' && address && by) {
      ...
      const [tradesRes, pnlRes] = await Promise.all([
        fetch(
          `${BASE_URL}/trades?account_index=${accountIndex}&limit=300&sort_by=timestamp&sort_dir=desc`,
        ),
