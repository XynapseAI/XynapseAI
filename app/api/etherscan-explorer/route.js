...
async function fetchCoinGeckoInfo(chain, addresses) {
    ...
    addresses.map(async (addr) => {
      ...
      try {
        ...
        if (res.data.id) {
          ...
          logger.info(
            `CoinGecko info for ${addr} on ${platform}: ${res.data.name} (${res.data.symbol})`,
          )
        }
      } catch (err) {
  ...
  logger.info(
    `CoinGecko info fetched for ${matchedCount} tokens on ${platform} (queried ${addresses.length})`,
  )
  return cgInfos
}
...
async function enrichWithCoinGecko(tokens) {
  ...
  return tokens.map((t) => {
    ...
    logger.info(
      `Enriched ${t.tokenAddress} on ${chain}: ${t.symbol || cgInfo?.symbol}, dec=${decimals}, amount=${amount.toFixed(6)}, USD=${valueUSD?.toFixed(2)}`,
    )
    return {
      ...t,
...
const supportedEtherscanChainIds = [
  '1',
  '42161',
  '137',
  '8453',
...
function isAllowedOrigin(origin, referer) {
  ...
  if (!origin && !referer) return true // Internal/SSR request
...
const bodySchema = z
  .object({
    ...
    txHash: z
      .string()
      .optional()
      .refine((val) => !val || /^0x[a-f0-9]{64}$/.test(val), {
  ...
  .refine(
    (data) =>
      ['wallet-balances', 'token-balances', 'transactions', 'address-overview'].includes(
        data.action,
      )
        ? !!data.address
        : true,
    {
      message:
        'Wallet address is required for wallet-balances, token-balances, transactions, and address-overview',
      path: ['address'],
    },
  )
  .refine((data) => (data.action === 'tx-details' ? !!data.txHash : true), {
    message: 'Transaction hash is required for tx-details',
    path: ['txHash'],
  })
  .refine((data) => (!['tx-details'].includes(data.action) ? !!data.chain : true), {
    message: 'Chain is required for this action',
    path: ['chain'],
  })
  .refine(
    (data) =>
      ['token-supply', 'token-info', 'token-transactions'].includes(data.action)
        ? !!data.tokenAddress
        : true,
    {
      message: 'Token address is required for token-supply, token-info and token-transactions',
      path: ['tokenAddress'],
    },
  )
// V2 unified base URL
  ...
  await Promise.all(
    calls.map(async (call) => {
      ...
      try {
        ...
        logger.warn(
          `Failed to fetch ${call.key} for token ${tokenAddress} on chain ${chainId}: ${err.message} - using fallback`,
        )
      }
    }),
  )
...
const handlerWrapper = (handler) =>
  limiterBottleneck.wrap(async (req) => {
    ...
    logger.info(
      `Request to /api/etherscan-explorer from IP ${ip}, Origin: ${origin || 'null'}, Referer: ${referer || 'null'}`,
    )
    ...
    res.headers.set('Access-Control-Allow-Origin', safeAllowOrigin)
...
export const POST = handlerWrapper(async (request) => {
  ...
  if (!process.env.ETHERSCAN_API_KEY) {
    ...
    return NextResponse.json(
      { detail: 'Server configuration error: Missing ETHERSCAN_API_KEY' },
      { status: 500 },
    )
  }
          ...
          if (action === 'tx-details' && txHash) {
            ...
            if (!isSupportedByEtherscan && chain) {
              ...
              if (data.tokenTransfers.length > 0) {
                ...
                data.tokenTransfers = data.tokenTransfers.map((t) => {
                  ...
                  logger.info(
                    `Token ${t.tokenAddress}: using decimals ${decimals} (${t.type || 'ERC20'})`,
                  )
                  return {
                    ...t,
                    ...info,
                ...
                logger.info(
                  `Found ${data.tokenTransfers.length} token transfers (${data.tokenTransfers.filter((t) => t.type === 'ERC721').length} NFTs), enriching on ${chain}`,
                )
              ...
              if (nativePrice) {
                ...
                logger.info(
                  `Native ${chain} enriched: value USD ${data.nativeValueUSD}, fee USD ${data.feeUSD}`,
                )
              }
            ...
            const searchPromises = sortedChains.map(async ([chainIdStr, chainName]) => {
              ...
              try {
                ...
                logger.info(
                  `TX search on ${chainName} (ID: ${chainIdStr}): status ${response.status}`,
                )
                if (response.data.result) {
                  ...
                  if (txChainId && txChainId !== chainIdStr) {
                    logger.warn(
                      `TX chainId mismatch on ${chainName}: expected ${chainIdStr}, got ${txChainId} - skipping`,
                    )
                    return null
                  }
            ...
            if (candidates.length === 0) {
              if (!chain) {
                ...
                if (retryRes.data.result) {
                  candidates = [
                    { chainName: baseName, chainId: baseId, transaction: retryRes.data.result },
                  ]
            ...
            const verifyPromises = candidates.map(async (cand) => {
              ...
              if (verify.valid) {
                logger.info(
                  `Verified valid tx on ${cand.chainName} (ID: ${cand.chainId}): ${verify.numLogs} logs`,
                )
            ...
            if (validCandidates.length > 1) {
              ...
              logger.warn(
                `Multiple valid candidates for ${txHash.slice(0, 10)}... taking max logs: ${validCandidates[0].chainName} (${validCandidates[0].numLogs} logs)`,
              )
            }
            const {
            ...
            logger.info(
              `TX verified on ${foundChainName} (ID: ${foundChainId}) for hash ${txHash.slice(0, 10)}... (${numLogs} logs)`,
            )
            let receiptResponse
            if (preReceipt) {
            ...
            logger.info(
              `Internal txs fetch for ${foundChainName}: status ${internalResponse.status}`,
            )
            ...
            if (data.tokenTransfers.length > 0) {
              ...
              await Promise.all(
                uniqueTokens.map(async (addr) => {
                  tokenInfos[addr.toLowerCase()] = await fetchTokenInfoViaProvider(
                    null,
                    addr,
                    foundChainId,
                  )
              ...
              logger.info(
                `Found ${data.tokenTransfers.length} token transfers (${data.tokenTransfers.filter((t) => t.type === 'ERC721').length} NFTs), enriching on ${foundChainName}`,
              )
              data.tokenTransfers = await enrichWithCoinGecko(data.tokenTransfers)
            } else {
              logger.info('No token transfers, skipping enrich')
            ...
            if (nativePrice) {
              ...
              const effectiveGasPrice = parseInt(
                data.receipt?.effectiveGasPrice || data.transaction.gasPrice || '0x0',
                16,
              )
              const fee = (gasUsed * effectiveGasPrice) / 1e18
              data.feeUSD = fee * nativePrice
            ...
            if (response.data.status === '1' && Array.isArray(response.data.result)) {
              ...
              response.data.result.forEach((tx) => {
                ...
                if (tx.to.toLowerCase() === address.toLowerCase()) {
                  balances[contract].balanceRaw = (
                    BigInt(balances[contract].balanceRaw) + value
                  ).toString()
                }
            ...
            controller.enqueue(
              JSON.stringify({ detail: `Invalid parameters for action: ${action}` }),
            )
          }
          controller.close()
        } catch (error) {
