...
const useUserData = (session, csrfToken, setIsAnalyzing, isWorldMiniApp) => { // ADDED: isWorldMiniApp param
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      setUserData({
        ...
        hasMiniAppNotifications: result.hasMiniAppNotifications, // ADDED: Assume server adds this field
      });
  ...
  return { userData, loading, error, handleAnalyzeTweets, recaptchaRef, fetchUserData }; // ADDED: Expose fetchUserData for refresh
};
...
function DashboardInner() {
  ...
  const { userData, loading, error, fetchUserData } = useUserData(session, csrfToken, setIsAnalyzing, isWorldMiniApp); // MODIFIED: Pass isWorldMiniApp, expose fetchUserData
  ...
  const handleAddMiniApp = async () => {
    ...
    try {
      ...
      // ADDED: Refresh userData to update hasMiniAppNotifications
      await fetchUserData();
    } catch (error) {
  ...
  // NEW: Prompt to add Mini App on first login in Mini App/Base/Warpcast (use server check instead of localStorage)
  useEffect(() => {
    if (status === 'authenticated' && userData && !loading && !userData.hasMiniAppNotifications && (isBaseApp || isWarpcastMobile) && typeof sdk !== 'undefined' && sdk.actions && sdk.actions.addMiniApp) {
      handleAddMiniApp();
    }
  }, [status, userData, loading, isBaseApp, isWarpcastMobile, handleAddMiniApp]);
