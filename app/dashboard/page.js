...
function DashboardInner() {
  ...
  const { userData, loading, error } = useUserData(session, csrfToken, setIsAnalyzing, isWorldMiniApp)
  ...
  useEffect(() => {
    ...
    if (tab && ['market', 'etf', 'ai', 'profile', 'graph', 'watchlists', 'cluster', 'explorer'].includes(tab)) {
      setActiveTab(tab)
    }
  }, [searchParams, router])
  ...
  useEffect(() => {
    ...
    const isBaseUADetected = userAgent.includes('coinbasewallet') || userAgent.includes('cbwallet') || userAgent.includes('base') || userAgent.includes('coinbase')
    ...
    const miniAppDetected = (isSDKLoaded && (context === 'miniapp' || !!miniAppUser)) || isWarpcastDetected || sdkAvailable || isBaseAppDetected
    setIsMiniApp(miniAppDetected)
    if (isBaseAppDetected) setFallbackToManual(true)
    ...
    if (miniAppDetected && miniAppUser) safeLog('Mini App ready! User FID:', miniAppUser?.fid)
  ...
  const handleMiniAppQuickAuth = useCallback(async () => {
    ...
    try {
      ...
      if (!tokenResponse || !tokenResponse.token) throw new Error('No token in response')
      ...
      if (errorMsg.includes('SDK') || errorMsg.includes('origins') || errorMsg.includes('result') || errorMsg.includes('not available')) {
        ...
        setMiniAppAuthFailed(false)
        if (isBaseApp) setBaseAuthFailed(true)
      } else {
  ...
  }, [isMiniApp, status, session, miniAppAuthLoading, handleMiniAppQuickAuth, context, miniAppUser, isWarpcastMobile])
  ...
  const handleWorldQuickAuth = useCallback(async () => {
    ...
    try {
      if (!MiniKit.isInstalled()) throw new Error('World App not detected. Please open in World App.')
      ...
      safeLog('WalletAuth response:', { commandPayload, finalPayload })
      ...
      if (!verifyRes.ok || !verifyData.isValid) throw new Error(verifyData.message || 'SIWE verification failed')
  ...
  const handleAddMiniApp = async () => {
    ...
    try {
      ...
      if (response?.notificationDetails) safeLog('Notification details:', response.notificationDetails)
    } catch (error) {
  ...
  const handleFarcasterSuccess = async (result) => {
    try {
      const callbackUrl = pendingInviteCode ? `/dashboard?invite=${pendingInviteCode}` : '/dashboard'
      const res = await signIn('farcaster', { message: result.message, signature: result.signature, callbackUrl, redirect: false })
      if (res?.error) {
  ...
  const handleEmailSignIn = async (e) => {
    ...
    try {
      const callbackUrl = pendingInviteCode ? `/dashboard?invite=${pendingInviteCode}` : '/dashboard'
      const result = await signIn('email', { email: trimmedEmail, callbackUrl, redirect: false })
      if (result?.ok) setEmailSent(true)
      else toast.error(result?.error || 'Failed to send login email.', { position: 'top-center' })
    } catch (err) {
      ...
      >
        <div className="h-screen w-screen bg-gradient-to-br from-black to-gray-900 backdrop-blur-xs text-white overflow-x-hidden flex flex-col">
          ...
          <main className="flex-1 flex items-center justify-center overflow-hidden">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="w-full h-full flex items-center justify-center">
              {isMiniApp && miniAppAuthFailed && !fallbackToManual ? (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="w-full h-full p-4 md:p-0 flex items-center justify-center text-white font-satoshi relative">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative z-20 bg-black/60 backdrop-blur-xs p-6 md:p-10 border border-white/15 rounded-lg max-w-sm w-full mx-4 flex flex-col items-center shadow-2xl shadow-black/50">
                    <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl md:text-3xl font-bold text-white uppercase mb-3 text-center tracking-wide">Authentication Failed</motion.h1>
                    <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[11px] md:text-xs text-gray-500 mb-6 text-center leading-relaxed">QuickAuth failed. Please try again or contact support.</motion.p>
                    <button onClick={handleMiniAppQuickAuth} className="w-full px-4 py-2.5 border-2 border-white/15 bg-white/10 text-white rounded-2xl text-sm font-semibold transition-all duration-300 hover:border-white/30 hover:bg-white/20 flex items-center justify-center">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="w-full h-full p-4 md:p-0 flex items-center justify-center text-white font-satoshi relative">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative z-20 bg-black/60 backdrop-blur-xs p-6 md:p-10 border border-white/15 rounded-lg max-w-sm w-full mx-4 flex flex-col items-center shadow-2xl shadow-black/50">
                    <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl md:text-3xl font-bold text-white uppercase mb-3 text-center tracking-wide">Authentication Failed</motion.h1>
                    <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[11px] md:text-xs text-gray-500 mb-6 text-center leading-relaxed">
                      {worldAppVersionOk ? 'World Auth failed. Please try again or contact support.' : 'Please update World App to v2.8.79 or higher.'}
                    </motion.p>
                    <button onClick={handleWorldQuickAuth} className="w-full px-4 py-2.5 border-2 border-white/15 bg-white/10 text-white rounded-2xl text-sm font-semibold transition-all duration-300 hover:border-white/30 hover:bg-white/20 flex items-center justify-center">
                      <MatrixHoverEffect text="Retry Authentication" hoverColor="#FFFFFF" />
                    </button>
                  </motion.div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="w-full h-full p-4 md:p-0 flex items-center justify-center text-white font-satoshi relative">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative z-20 bg-black/60 backdrop-blur-xs p-6 md:p-10 border border-white/15 rounded-lg max-w-sm w-full mx-4 flex flex-col items-center shadow-2xl shadow-black/50">
                    <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl md:text-3xl font-bold text-white uppercase mb-3 text-center tracking-wide">Authentication Failed</motion.h1>
                    <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[11px] md:text-xs text-gray-500 mb-6 text-center leading-relaxed">Base App deeplink failed. Please try again.</motion.p>
                    <button onClick={handleBaseSiweAuth} disabled={baseAuthLoading} className="w-full px-4 py-2.5 border-2 border-white/15 bg-white/10 text-white rounded-2xl text-sm font-semibold transition-all duration-300 hover:border-white/30 hover:bg-white/20 flex items-center justify-center">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="w-full h-full p-4 md:p-0 flex items-center justify-center text-white font-satoshi relative">
                  {isBaseApp ? (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative z-20 bg-black/60 backdrop-blur-xs p-6 md:p-10 border border-white/15 rounded-lg max-w-sm w-full mx-4 flex flex-col items-center shadow-2xl shadow-black/50">
                      <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl md:text-3xl font-bold text-white uppercase mb-3 text-center tracking-wide">Sign In/Sign Up</motion.h1>
                      <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[11px] md:text-xs text-gray-500 mb-6 text-center leading-relaxed">Click the button below to start a secure login via deeplink in the Base App.</motion.p>
                      <button onClick={handleBaseSiweAuth} disabled={baseAuthLoading} className="w-full px-4 py-2.5 border border-white/80 bg-transparent text-white/80 rounded-sm text-sm font-semibold transition-all duration-300 hover:border-white/30 hover:bg-white/20 flex items-center justify-center">
                        {baseAuthLoading ? <BlinkingDots /> : (
                          <>
                      ...
                      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="mt-4 text-[11px] text-gray-500 text-center leading-relaxed">
                        By clicking continue, you agree to the{' '}
                        <button onClick={() => openModal('terms')} className="text-white hover:underline">Terms of Service</button>{' '}
                        and{' '}
                        <button onClick={() => openModal('privacy')} className="text-white hover:underline">Privacy Policy</button>.
                      </motion.p>
                    </motion.div>
                  ) : (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative z-20 bg-black/60 backdrop-blur-xs p-6 md:p-10 border border-white/15 rounded-lg max-w-sm w-full mx-4 flex flex-col items-center shadow-2xl shadow-black/50">
                      <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl md:text-3xl font-bold text-white uppercase mb-3 text-center tracking-wide">Sign In</motion.h1>
                      <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[11px] md:text-xs text-gray-500 mb-6 text-center leading-relaxed">Access your dashboard with secure authentication.</motion.p>
                      {!isWorldMiniApp && (
                        <>
                          <form onSubmit={handleEmailSignIn} className="w-full space-y-4">
                            <input type="email" value={email} onChange={(e) => !emailSent && setEmail(e.target.value)} placeholder="Enter your email" className="w-full px-4 py-2.5 bg-black/60 border border-white/15 rounded-lg text-gray-500 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white/30 transition-all duration-300" required disabled={sendingEmail || emailSent} />
                            <button type="submit" disabled={sendingEmail || emailSent} className="w-full px-4 py-2.5 bg-white/90 text-black rounded-lg text-sm font-semibold transition-all duration-300 hover:bg-white/70 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center">
                              {sendingEmail ? <>Sending…</> : emailSent ? <>Sent</> : <MatrixHoverEffect text="Sign in with Email" hoverColor="#FFFFFF" />}
                            </button>
                            {emailSent && <p className="text-center text-[10px] text-emerald-400/80 mt-2">Please check your inbox and spam folder!</p>}
                          </form>
                      ...
                      {providers?.google && !isWorldMiniApp && (
                        <button onClick={handleGoogleSignIn} className="m-4 w-full px-4 py-2.5 bg-black/20 border border-white/25 rounded-lg text-white text-sm font-semibold flex items-center justify-center gap-3 transition-all duration-300 hover:bg-gray-800/30 hover:border-white/40">
                      ...
                      {!isWorldMiniApp && (isWarpcastMobile || (isBaseApp && fallbackToManual)) && (
                        <SignInButton
                          ...
                          style={{ display: 'flex !important', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.2) !important', border: '1px solid rgba(255,255,255,0.25) !important', color: 'white !important', borderRadius: '1rem !important', transition: 'all 0.3s !important' }}
                          buttonText="Sign in with Farcaster"
                          showLogo={true}
                        >
                          <Image src="/logos/farcaster-logo.webp" alt="Farcaster Logo" width={20} height={20} className="w-6 h-6 rounded-xl object-contain mr-2" />
                      ...
                      {error && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-4 text-red-300 text-sm bg-red-500/10 border border-red-500/20 rounded-lg p-2 text-center">
                          Error: {error}
                        </motion.div>
                      )}
                      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="mt-4 text-[11px] text-gray-500 text-center leading-relaxed">
                ...
                <>
                  {requiresAuth && status === 'unauthenticated' ? (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 text-center text-gray-400 max-w-md">
                      <h2 className="text-xl font-bold mb-4">Login Required</h2>
                      <p>Sign in to access {activeTab} features. Xynapse offers advanced blockchain analytics for crypto enthusiasts.</p>
                    </motion.div>
                  ) : (
                    <>
                      {activeTab === 'market' && <MarketTab recaptchaRef={recaptchaRef} toast={toast} onTokenSelect={handleNavigateToToken} initialTokenSlug={searchParams.get('token') || undefined} />}
                      {activeTab === 'etf' && <EtfTab />}
                      {activeTab === 'cluster' && <ClusterTab recaptchaRef={recaptchaRef} initialClusterId={searchParams.get('clusterId') || 'binance'} />}
                      {activeTab === 'graph' && <TreemapTab onTokenSelect={handleNavigateToToken} />}
                      {activeTab === 'ai' && <AITab recaptchaRef={recaptchaRef} />}
                      {activeTab === 'explorer' && <ExplorerTab initialQuery={searchParams.get('query')} initialChain={searchParams.get('chain')} />}
                      {activeTab === 'dex' && <DexTab />}
                      {activeTab === 'profile' && <ProfileTab userData={userData} loading={loading} error={error} isConnected={walletConnected} handleConnectWallet={handleConnectWallet} recaptchaRef={recaptchaRef} handleSignOut={handleSignOut} />}
          ...
          {process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY ? (
            <ReCAPTCHA ref={recaptchaRef} sitekey={process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY} size="invisible" badge="bottomright" onError={() => toast.error('Failed please refresh the App', { position: 'top-center' })} />
          ) : (
            <p className="text-[8px] text-red-600 ml-2">Error: reCAPTCHA site key is missing. Please configure NEXT_PUBLIC_RECAPTCHA_SITE_KEY.</p>
          )}
          <p className="text-[8px] text-gray-600 ml-2">
            Protected by reCAPTCHA. See{' '}
            <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer" className="text-neon-blue">Privacy Policy</a> &{' '}
            <a href="https://policies.google.com/terms" target="_blank" rel="noopener noreferrer" className="text-neon-blue">Terms</a> of Google.
          </p>

          <ToastContainer position="top-center" autoClose={3000} hideProgressBar={false} newestOnTop={false} closeOnClick rtl={false} pauseOnFocusLoss draggable pauseOnHover theme="dark" limit={3} toastStyle={{ backgroundColor: 'rgba(0, 0, 0, 0.9)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '16px', color: '#FFF' }} />

          {isModalOpen && (
            <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50" onClick={closeModal}>
              <div className="bg-gray-900/50 backdrop-blur-lg border border-white/20 rounded-2xl w-full max-w-7xl h-[90vh] relative flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="sticky top-0 z-10 backdrop-blur-lg border-b border-white/20 p-6 flex justify-between items-center">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white uppercase">
                    ...
                    <span className="block text-sm sm:text-base text-gray-300 mt-1">Effective Date: June 21, 2025</span>
                  </h1>
                  <button onClick={closeModal} aria-label="Close modal" className="text-white text-xl font-bold hover:text-neon-blue transition-all duration-300">✕</button>
                </div>
...
// ==============================================
// BASE ONLY DASHBOARD - BASE APP
// ==============================================
function BaseOnlyDashboardInner() {
  useEffect(() => {
    const handleUnhandledRejection = (event) => {
  ...
  // Base App luôn là true
  const isBaseApp = true
  const isMiniApp = false
  const isWarpcastMobile = false
  const [baseAuthLoading, setBaseAuthLoading] = useState(false)
  const [baseAuthFailed, setBaseAuthFailed] = useState(false)
  const [isWorldMiniApp, setIsWorldMiniApp] = useState(false)
  const [worldAppVersionOk, setWorldAppVersionOk] = useState(true)
  ...
  useEffect(() => {
    if (isMounted && !providers) fetchProvidersWithRetry()
  }, [isMounted, providers, fetchProvidersWithRetry])
  ...
  const handleBaseSiweAuth = useCallback(async () => {
    ...
    try {
      if (!walletConnected || !walletAddress) {
        const coinbaseConnector = coinbaseWallet({ appName: 'XynapseAI', preference: 'smartWalletOnly' })
        await connectWallet({ connector: coinbaseConnector })
        await new Promise((resolve) => setTimeout(resolve, 1500))
      }
      const nonceRes = await fetch('/api/nonce')
      ...
      const messageStr = siweMessage.prepareMessage()
      const signature = await signMessageAsync({ message: messageStr })
      const result = await signIn('base', { redirect: false, message: messageStr, signature, callbackUrl: '/dashboard' })
      if (result?.error) throw new Error(result.error)
      setAuthSuccess(true)
      await update()
    } catch (err) {
  ...
  const handleSignOut = async () => {
    ...
    try {
      if (typeof window !== 'undefined') await clearAllCaches(session.user.id)
      await signOut({ redirect: false })
      try {
        const currentCsrfToken = csrfToken || session.csrfToken
        if (currentCsrfToken) {
  ...
  // Loading state cho Base App
  if (!isMounted || !providers || status === 'loading' || worldAuthLoading) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-black text-white">
  ...
  const requiresAuth = ['profile', 'ai', 'watchlists'].includes(activeTab)
  const showLoginForm = status === 'unauthenticated' && !authSuccess && !baseAuthFailed

  return (
    <CurrencyProvider>
      {isBaseApp && <SafeArea />}
      <div className="h-screen w-screen bg-gradient-to-br from-black to-gray-900 backdrop-blur-xs text-white overflow-x-hidden flex flex-col">
        <Header activeTab={activeTab} setActiveTab={setActiveTab} handleSignOut={handleSignOut} selectedAddress={searchParams.get('address') || undefined} />
        <main className="flex-1 flex items-center justify-center overflow-hidden">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="w-full h-full flex items-center justify-center">
            {baseAuthFailed ? (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="w-full h-full p-4 md:p-0 flex items-center justify-center text-white font-satoshi relative">
...
// ==============================================
// ROOT DASHBOARD - CONDITIONAL RENDER
// ==============================================
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const isBaseDetected = ua.includes('coinbase') || ua.includes('cbwallet') || ua.includes('base')
    setIsBaseApp(isCoinbaseWallet || isBaseDetected)
  }, [])
