...
const validateGet = [
    ...
    expressQuery('interactionType').isString().isIn(['chat', 'market']).optional().withMessage('Loại tương tác không hợp lệ'),
];
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const dailyInteraction = result.rows.length > 0 ? result.rows[0] : { count: 0 };
