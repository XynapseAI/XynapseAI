...
const normalizeTimestamp = (tx) => {
  ...
  const chain =
    typeof tx.chain === 'string'
      ...
      : (tx.chain_id || 'unknown').toString().toLowerCase()
      ...
      >
        ...
        <p className="text-[#FFF] font-semibold">
          ...
          <span className="text-emerald-400">
            {formatLargeNumber(payload[0].value, currency, 2)} {currency.toUpperCase()}
          </span>
        </p>
      </motion.div>
    )
...
const ClusterTab = ({
  ...
  const fetchClusterImage = async (clusterId) => {
    ...
    try {
      ...
      if (response.ok && result.success && result.data && result.data.length > 0) {
        ...
        logger.warn(`No cluster image found in DB for: ${clusterId}, using fallback`, {
          shortQuery,
        })
      }
    } catch (err) {
      logger.error(`Error fetching cluster image for ${clusterId}:`, {
        error: err.message,
        stack: err.stack,
      })
      const fallback = `/icons/${shortQuery}.webp`
  ...
  const fetchExchangeData = async (originalId, mappedId) => {
    ...
    try {
      ...
      const trustScore =
        result.data.trust_score !== undefined && result.data.trust_score !== null
      ...
      logger.log('Fetched and cached exchange data:', {
        mappedId,
      ...
      await setCachedData(cacheKey, fallback)
      logger.error('Error fetching exchange data:', {
        originalId,
        mappedId,
        error: errorMessage,
        stack: err.stack,
  ...
  const fetchVolumeHistory = async (exchangeId) => {
    ...
    try {
      ...
      logger.error('Error fetching volume history:', {
        exchangeId,
        error: errorMessage,
        stack: err.stack,
      })
  ...
  const fetchPortfolioAndWallets = async (clusterId) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/token-cluster?exchange=${encodeURIComponent(clusterId)}&currency=${encodeURIComponent(currency)}`,
        {
          headers,
          credentials: 'include',
      ...
      if (!result.success) {
        throw new Error(
          result.detail || `No portfolio or wallet data found for cluster: ${clusterId}`,
        )
      }
        ...
        startTransition(() => {
          ...
          setWalletData(
            result.wallets.map((wallet) => ({
              ...wallet,
              cluster_name:
                wallet.cluster_name || clusterId.charAt(0).toUpperCase() + clusterId.slice(1),
              image:
                wallet.image || `/icons/${clusterId.toLowerCase()}.webp` || '/fallback-image.webp',
            })) || [],
          )
        })
      ...
      logger.error('Error fetching portfolio/wallet data:', {
        clusterId,
        error: errorMessage,
        stack: err.stack,
      })
  ...
  const {
    data: bitcoinTxs,
    error: bitcoinError,
    isValidating: bitcoinValidating,
  } = useSWR(
    status === 'authenticated' && clusterIdFromQuery && walletData.length > 0
      ? ['bitcoin-transactions', clusterIdFromQuery, walletData.length]
      : null,
    async () => {
      const mappedId = mapExchangeId(clusterIdFromQuery)
      ...
      logger.log('Fetched and cached Bitcoin transactions:', {
  ...
  const topEvmWallets = useMemo(() => {
    ...
    const evmWallets = uniqueWalletData
      ...
      .filter(Boolean)
    logger.info(`Optimized EVM wallets for transactions: ${evmWallets.length} top wallets`, {
      clusterId: clusterIdFromQuery,
    })
    return evmWallets
  ...
  const {
    data: evmTxs,
    error: evmError,
    isValidating: evmValidating,
  } = useSWR(
    status === 'authenticated' && topEvmWallets.length > 0
      ? ['evm-transactions', topEvmWallets.join(','), currency]
      : null,
    async () => {
      ...
      logger.log('Fetched and cached EVM transactions:', {
        topEvmWallets: topEvmWallets.length,
        count: result.length,
      })
      return result
    },
  ...
  useEffect(() => {
    if (evmError || bitcoinError) {
      const errorMessage =
        evmError?.message || bitcoinError?.message || 'Failed to load transactions'
  ...
  const {
    data: walletTxData,
    error: walletTxError,
    isValidating: walletTxValidating,
  } = useSWR(
  ...
  const {
    ...
    error: walletBalancesErrorSWR,
    isValidating: walletBalancesValidating,
  } = useSWR(
  ...
  }, [status, clusterIdFromQuery, currency])
    ...
    if (status === 'authenticated' && selectedWallet) {
      ...
      if (/^0x[a-fA-F0-9]{40}$/.test(selectedWallet)) {
        ...
        setWalletBalancesError(
          'Balance data not available for non-EVM addresses (Bitcoin, Dogecoin, Litecoin)',
        )
        setWalletTransactionsError(
          'Transaction data not available for non-EVM addresses (Bitcoin, Dogecoin, Litecoin)',
        )
      }
    }
  ...
  const handleWalletClick = (walletObj) => {
    ...
    if (!address) {
      toast.info('No on-chain address available for this entity', {
        position: 'top-center',
        autoClose: 3000,
      })
      return
    ...
    const isValidAddress =
      ...
      /^L[a-km-zA-HJ-NP-Z1-9]{26,34}$|^ltc1[a-zA-Z0-9]{39,59}$/.test(address)
    if (!isValidAddress) {
      toast.info('No on-chain data available for this nametag', {
        position: 'top-center',
        autoClose: 3000,
      })
      return
  ...
  const groupedPortfolio = useMemo(() => {
    logger.log('Processing portfolio data for chain details:', {
      portfolioData: memoizedPortfolioData,
    })
    const totalValue = memoizedPortfolioData.reduce(
      (sum, item) => sum + (Number(item.total_balance_usd) || 0),
      0,
    )
  ...
  const truncateAddressWithHover = (address, nameTag, source) => {
    ...
    if (source === 'Blockchair') {
      ...
      const { text, shortAddress: computedShortAddress } = truncateAddress(
        address,
        { [normalizedAddress]: { name: nameTag } },
        source,
      )
          ...
          {isLoadingPortfolio ? (
            ...
            <table className="w-full table-fixed text-[9px] sm:text-[11px] bg-[#0A0A0A]/80 rounded-xl">
              <thead className="border-b border-[#FFFFFF10] bg-[#0A0A0A]/80 backdrop-blur-md">
                <tr>
                  <th
                    className={`${isMobile ? 'w-[25%]' : 'w-[25%]'} px-3 py-2 text-[#FFF] text-left font-semibold truncate`}
                  >
                    Token
                  </th>
                  <th
              ...
              <tbody>
                {groupedPortfolio.map((group, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-[#FFF] truncate">
                      <span className="font-semibold">
                        {formatPrice(group.total_balance_usd || 0, currency, 2)}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-[#FFF] truncate">
                      <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] sm:text-sm text-[#D4D4D4] text-center py-4">
              No portfolio data available for this cluster.
            </p>
          )}
        </div>
      </div>
  ...
  const renderWalletsContent = () => {
    ...
    const totalValue = uniqueWalletData.reduce(
      (sum, wallet) => sum + (Number(wallet.total_value_usd) || 0),
      0,
    )
    return (
      <div className="relative bg-[#0A0A0A]/80 backdrop-blur-md overflow-y-auto min-h-[calc(50vh)] sm:min-h-[calc(30vh)] max-h-[calc(50vh)] sm:max-h-[calc(50vh-5rem)] hide-scrollbar">
        {isLoadingWallets && (
          <LoadingOverlay isLoading={isLoadingWallets} isMobile={isMobile} className="z-10" />
        )}
        {isLoadingWallets ? (
          <SkeletonLoader count={5} isMobile={isMobile} />
          ...
          <table className="w-full table-fixed text-[9px] sm:text-[11px] bg-[#0A0A0A]/80 rounded-xl">
            <thead className="border-b border-[#FFFFFF10] bg-[#0A0A0A]/80 backdrop-blur-md">
              <tr>
                <th
                  className={`${isMobile ? 'w-[50%]' : 'w-[50%]'} px-3 py-2 text-[#FFF] text-left font-semibold truncate`}
                >
                  Wallet
                </th>
                <th
            ...
            <tbody>
              {uniqueWalletData.map((wallet, index) => {
                const percentage =
                  totalValue > 0 ? ((Number(wallet.total_value_usd) || 0) / totalValue) * 100 : 0
                const chainLower = wallet.chain?.toLowerCase()
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              const addr = wallet.holder_address || wallet.name_tag
                              if (addr && wallet.chain) {
                                const url = getInternalExplorerUrl(wallet.chain, '', addr, 'wallet')
                                window.open(url, '_blank', 'noopener,noreferrer')
                              }
                            }}
                            className="text-left w-full"
                          >
                            {truncateAddressWithHover(
                              wallet.holder_address,
                              wallet.display_name,
                              chainLower === 'bitcoin' ? 'Blockchair' : undefined,
                            )}
                          </button>
                        </div>
                      </div>
                    ...
                    <td className="px-3 py-2.5 text-[#FFF] truncate">
                      <span className="font-semibold">
                        {formatPrice(Number(wallet.total_value_usd) || 0, currency, 2)}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-[#FFF] truncate">
          ...
          <p className="text-[10px] sm:text-sm text-[#D4D4D4] text-center py-4">
            No wallet data available for this cluster.
          </p>
        )}
      </div>
    )
  ...
  const renderTransactionsContent = () => {
    ...
    const renderTransactionRow = (index, tx) => {
      ...
      const fromWallet =
        uniqueWalletData.find(
          (w) => (w.holder_address || w.name_tag)?.toLowerCase() === tx.from?.toLowerCase(),
        ) || {}
      const toWallet =
        uniqueWalletData.find(
          (w) => (w.holder_address || w.name_tag)?.toLowerCase() === tx.to?.toLowerCase(),
        ) || {}
      const fromNtag = {
      ...
      const directionColor = isInternal
        ? 'text-[#FFF]'
        : isOutgoing
          ? 'text-red-400'
          : isIncoming
        ...
        >
          <div className="w-[20%] sm:w-[20%] px-2 sm:px-3 text-[#FFF]/80 text-[9px] sm:text-[10px] text-center overflow-hidden text-ellipsis">
            <div className="flex flex-col items-center justify-center gap-1 relative">
              ...
              <span className="text-[7px] sm:text-[9px] truncate max-w-[60px] sm:max-w-[80px]">
                {tokenSymbol}
              </span>
            </div>
          </div>
          <div className="w-[35%] sm:w-[35%] px-2 sm:px-3 text-[#FFF]/80 text-[8px] sm:text-[10px] flex items-center">
            <div className="flex items-center gap-2 w-full">
              <div className="flex-shrink-0">
                ...
                >
                  <path
                    d="M10 0 V22 M10 22 L4 16 M10 22 L16 16"
                    stroke="#00e77bff"
                    strokeWidth="3"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                ...
                <div className="flex items-center gap-1.5 group">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      if (tx.from && chain) {
                        const url = getInternalExplorerUrl(chain, '', tx.from, 'wallet')
                        window.open(url, '_blank', 'noopener,noreferrer')
                      }
                    }}
                ...
                <div className="flex items-center gap-1.5 group mt-0.5">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      if (tx.to && chain) {
                        const url = getInternalExplorerUrl(chain, '', tx.to, 'wallet')
                        window.open(url, '_blank', 'noopener,noreferrer')
                      }
                    }}
            ...
            <div className="flex flex-col items-center gap-0.5">
              <button
                onClick={(e) => {
                  ...
                  if (hash && chain) {
                    const url = getInternalExplorerUrl(chain, hash)
                    window.open(url, '_blank', 'noopener,noreferrer')
                  }
                }}
        ...
        {isLoadingTransactions ? (
          <div className="w-full table-fixed text-[9px] sm:text-[11px]">
            <div className="border-b border-[#FFFFFF10] bg-[#0A0A0A]/80 backdrop-blur-md flex">
              <div className="w-[20%] sm:w-[20%] px-3 py-2 text-[#FFF] font-medium text-center">
                Token
              </div>
              <div className="w-[35%] sm:w-[35%] px-3 py-2 text-[#FFF] font-medium text-center">
                From/To
              </div>
              <div className="w-[25%] sm:w-[25%] px-3 py-2 text-[#FFF] font-medium text-center">
                Value (Token)
              </div>
              <div className="w-[20%] sm:w-[20%] px-3 py-2 text-[#FFF] font-medium text-center">
                Details
          ...
          <p className="text-[10px] sm:text-xs text-[#D4D4D4] text-center">
            No large transactions available for this cluster.
          </p>
        )}
      </div>
    )
  ...
  const getInternalExplorerUrl = (chain, txHash = '', address = '', type = '') => {
    ...
    // Danh sách chain mà explorer nội bộ hỗ trợ
    ...
    // Fallback cũ (nếu cần giữ lại logo)
  ...
  return (
    ...
    >
      ...
      {error && (
        ...
        >
          Some entities without complete organization information may not be displayed. Note:
          Token/coin data is aggregated from on-chain sources and is for reference only. Certain
          clusters or organizations may have incomplete data.
        </motion.div>
      )}
        ...
        >
            ...
            {isLoadingExchange || isLoadingPrices ? (
              ...
              <div>
                  ...
                  <div className="flex items-center gap-2">
                    <motion.img
                      src={
                        exchangeData.image ||
                        clusterImage ||
                        `/icons/${mapExchangeId(clusterIdFromQuery).toLowerCase()}.webp` ||
                        '/fallback-image.webp'
                      }
                      ...
                      transition={{ duration: 0.2 }}
                    />
                    <h4 className="ml-2 text-lg sm:text-xl font-bold text-[#FFF] uppercase tracking-wide">
                      {exchangeData.name}
                    </h4>
                  </div>
                  <motion.div
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <motion.div className="bg-[#FFFFFF]/5 rounded-xl p-3 border border-[#FFFFFF10] hover:border-[#FFFFFF20] transition-colors">
                    ...
                    <div className="space-y-2 text-[10px] sm:text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#FFF]">
                          {exchangeData.country || 'Not available'}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-1 rounded-lg text-xs font-semibold ${getTrustScoreBadge(exchangeData.trust_score)}`}
                        >
                  ...
                  <motion.div className="bg-[#FFFFFF]/5 rounded-xl p-3 border border-[#FFFFFF10] hover:border-[#FFFFFF20] transition-colors">
                    ...
                    <div className="space-y-2 text-[10px] sm:text-xs">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#FFF]">
                          {exchangeData.centralized ? 'Yes' : 'No'}
                        </span>
                      </div>
                    </div>
                  </motion.div>
              ...
              <p className="text-[10px] sm:text-xs text-[#D4D4D4] text-center">
                No cluster data available. Please select another cluster.
              </p>
            )}
          </div>
          <div className="flex-1 p-2 sm:p-4 mr-2 sm:mr-0">
            ...
            {isLoadingVolume ? (
                ...
                >
                  ...
                  <defs>
                    <linearGradient id="chartGradient" x1="0" y1="0" x2="0" y2="1">
                      ...
                      {/* CHANGED: Neon green gradient for volume */}
              ...
              <p className="text-[10px] sm:text-xs text-[#D4D4D4] text-center">
                No volume data available for this cluster.
              </p>
            )}
          </div>
        </motion.div>
            ...
            <div className="p-0 border-b border-[#FFFFFF10] bg-[#0A0A0A]/80 backdrop-blur-md flex gap-4 items-end h-[52px]">
              <motion.button
                onClick={() => {
                  ...
                  router.push(
                    `${window.location.pathname}?tab=cluster&subtab=portfolio&clusterId=${encodeURIComponent(clusterIdFromQuery)}`,
                    { scroll: false },
                  )
                }}
              ...
              >
                ...
                >
                  <path
                    d="M12 2V12H2C2 6.47715 6.47715 2 12 2Z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12H12V2C17.5228 2 22 6.47715 22 12Z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <motion.button
                onClick={() => {
                  ...
                  // Update URL
                  router.push(
                    `${window.location.pathname}?tab=cluster&subtab=wallets&clusterId=${encodeURIComponent(clusterIdFromQuery)}`,
                    { scroll: false },
                  )
                }}
