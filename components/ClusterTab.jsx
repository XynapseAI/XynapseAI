...
const ClusterTab = ({ recaptchaRef, initialExchangeId }) => {
  ...
  const loadRecaptchaScript = () => {
    ...
    script.onload = () => {
      window.grecaptcha.ready(() => {
        logger.info("reCAPTCHA script loaded and ready");
        setRecaptchaLoaded(true);
      });
    };
    script.onerror = () => {
      ...
      toast.error("Failed to load reCAPTCHA script", { position: "top-center", autoClose: 3000 });
    };
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "development") {
      ...
      setRecaptchaLoaded(true); // Bỏ qua ở môi trường development
    }
  }, []);

  // Chỉ gọi fetchPortfolioAndWallets khi reCAPTCHA đã sẵn sàng
  useEffect(() => {
    if (recaptchaLoaded) {
      const mappedId = mapExchangeId(exchangeIdFromQuery);
  ...
  }, [exchangeIdFromQuery, currency, recaptchaLoaded]);
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'development') {
        if (!recaptchaRef.current) {
          logger.error('reCAPTCHA ref is not available', { exchangeId });
          ...
          try {
            await new Promise((resolve, reject) => {
              if (window.grecaptcha && window.grecaptcha.ready) {
                ...
                logger.warn('reCAPTCHA not ready, waiting...', { attempt: i + 1 });
                setTimeout(() => reject(new Error('reCAPTCHA script not ready')), 5000);
              }
            });
            ...
            logger.warn(`reCAPTCHA token generation failed (attempt ${i + 1}): ${err.message}`, { exchangeId });
            if (i === maxRetries - 1) {
      ...
      logger.info('Sending API request to /api/token-cluster', { exchangeId, headers: { ...headers, Authorization: headers.Authorization ? '[REDACTED]' : undefined } });
