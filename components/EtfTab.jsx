                ...
                {payload.map((entry, i) => {
                    ...
                    return (
                        <div key={i} className="flex items-center gap-3">
                            ...
                            <span
                                className={`font-bold ${isOutflow ? 'text-red-500/80' : 'text-emerald-400'}`}
                            >
...
const ManualLegend = ({ activeSymbols, setActiveSymbols, topSymbols, symbolToColor }) => {
    const toggleSymbol = (symbol) => {
        ...
        setActiveSymbols((prev) => {
            const newActive = prev.includes(symbol)
...
export default function EtfTab() {
    ...
    useEffect(() => {
        fetch('/api/etf-data')
            ...
            .then((data) => {
                ...
                setActiveSymbols(
                    data.chartArray.length > 0
                        ? Object.keys(data.chartArray[0]).filter((k) => k !== 'date')
                        : [],
                )
            })
    ...
    if (error)
        return (
            <div className="flex justify-center items-center h-full text-red-500/80">
                Error: {error}
            </div>
        )
    ...
    return (
        ...
        >
            ...
            {/* Right: Table */}
            <motion.div
                className="w-full md:w-2/5 min-h-[500px] md:min-h-[550px] max-h-[644px] border border-[#FFFFFF20] rounded-2xl bg-[#0A0A0A]/90 backdrop-blur-xl shadow-2xl glow-[#FFFFFF10] p-6 overflow-y-auto overflow-x-hidden custom-scrollbar"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.3 }}
            >
                <h3 className="text-sm font-bold text-[#FFF] mb-6">
                    Top Bitcoin ETF Holdings & Flows
                </h3>
                    ...
                    {tableData.map((etf, i) => (
                        ...
                        >
                            <div className="flex flex-col items-start gap-1 flex-shrink-0">
                                ...
                                />
                                <div className="text-xs sm:text-sm font-bold text-white m-1">
                                    {etf.name}
                                </div>
                                <div className="text-xs text-[#D4D4D4]">
                                    Ticker:{' '}
                                    <span className="text-emerald-400 font-bold">{etf.symbol}</span>
                                </div>
                            </div>
                            <div className="flex-1 w-full sm:ml-auto sm:text-right text-xs space-y-0.5 sm:space-y-1">
                                ...
                                <div className="text-sm sm:text-base font-bold text-white">
                                    {(etf.totalHolding / 1000).toFixed(3)} BTC
                                </div>
                                <div className="text-xs text-[#888]">
                                    {formatPrice(etf.valueUSD)}
                                </div>
                                <div
                                    className={
                                        etf.inflow > 0
