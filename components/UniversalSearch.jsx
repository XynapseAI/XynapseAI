...
export default function UniversalSearch({
  ...
  const [groupedResults, setGroupedResults] = useState({
    wallets: [],
    organizations: hideOrganizations ? [] : [],
    tokens: [],
    nametags: [],
    clusters: [],
  })
  ...
  // SWR hooks
  const { data: nametagData, isLoading: isLoadingNametags } = useSWR(
  ...
  const { data: exchangeData, isLoading: isLoadingExchanges } = useSWR(
    debouncedQuery
      ? `/api/coingecko?action=exchange-search&query=${encodeURIComponent(debouncedQuery)}`
      : null,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 30000, refreshInterval: 300000 },
  )

  const { data: clusterData, isLoading: isLoadingClusters } = useSWR(
    debouncedQuery ? `/api/search-clusters?query=${encodeURIComponent(debouncedQuery)}` : null,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 30000, refreshInterval: 300000 },
  )

  const { data: tokenData, isLoading: isLoadingTokens } = useSWR(
  ...
  useEffect(() => {
    if (!isModalOpen) {
      setGroupedResults({ wallets: [], organizations: [], tokens: [], nametags: [], clusters: [] })
      setIsLoading(false)
      return
    }
    ...
    // 2. Major organizations (chỉ nếu không hide)
    ...
    if (nametagData?.success && nametagData.data) {
      ...
      if (isValidAddress(debouncedQuery)) {
        newGrouped.nametags = newGrouped.nametags.filter(
          (nt) => nt.address.toLowerCase() === debouncedQuery.toLowerCase(),
        )
      }
    }

    // 4. Exchanges (chỉ nếu không hide)
    if (exchangeData?.data && !hideOrganizations) {
    ...
    if (tokenData?.success && tokenData.data) {
      newGrouped.tokens.push(
        ...tokenData.data.map((token) => ({
          id: `token-${token.coingecko_id || token.contractAddress || token.symbol}`,
          type: 'token',
  ...
  const getResultIcon = (type) => {
    switch (type) {
      ...
      case 'cluster':
        return <Building2 size={config.icon} className="text-indigo-400" />
      default:
  ...
  const getTypeTagClass = (type) => {
    switch (type) {
      ...
      case 'cluster':
        return 'bg-gradient-to-r from-indigo-500/20 to-purple-600/20 text-indigo-300 border-indigo-500/30 backdrop-blur-sm shadow-indigo-500/10'
      default:
        return 'bg-gradient-to-r from-gray-500/20 to-slate-600/20 text-gray-300 border-gray-500/30 backdrop-blur-sm shadow-gray-500/10'
    }
  }
  ...
  const getChainTagClass = (chain) => {
    const chainColors = {
      ...
      arbitrum:
        'bg-gradient-to-r from-orange-500/20 to-red-600/20 text-orange-300 border-orange-500/30',
      polygon: 'bg-gradient-to-r from-pink-500/20 to-rose-600/20 text-pink-300 border-pink-500/30',
    }
    return (
      chainColors[chain?.toLowerCase()] ||
      'bg-gradient-to-r from-gray-500/50 to-slate-600/50 text-gray-500 border-gray-500/50 backdrop-blur-sm shadow-gray-500/10'
    )
  }
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 mobile-scroll relative">
                ...
                {/* Tokens */}
                {groupedResults.tokens.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-white font-bold text-sm mb-3">
                      {getSectionTitle('tokens')}
                    </h3>
                    {groupedResults.tokens.slice(0, 10).map((result) => (
                      <motion.button
                        key={result.id}
                      ...
                      >
                        <div className="flex items-center mr-3 flex-shrink-0">
                          ...
                          <motion.div style={{ display: result.image ? 'none' : 'flex' }}>
                ...
                {/* Wallets */}
                {groupedResults.wallets.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-white font-bold text-sm mb-3">
                      {getSectionTitle('wallets')}
                    </h3>
                    {groupedResults.wallets.slice(0, 10).map((result) => (
                      <motion.button
                        key={result.id}
                        onClick={() => handleResultSelect(result)}
                        className={`flex items-center w-full text-left px-3 py-3 hover:bg-white/10 text-white transition-all duration-300 border-b border-white/5 last:border-b-0 rounded-lg mx-1 my-1 hover:shadow-neon-sm ${config.modalResult}`}
                ...
                {groupedResults.organizations.length > 0 && !hideOrganizations && (
                  <div className="mb-6">
                    <h3 className="text-white font-bold text-sm mb-3">
                      {getSectionTitle('organizations')}
                    </h3>
                    {groupedResults.organizations.slice(0, 10).map((result) => (
                      <motion.button
                        key={result.id}
                        onClick={() => handleResultSelect(result)}
                        className={`flex items-center w-full text-left px-3 py-3 hover:bg-white/10 text-white transition-all duration-300 border-b border-white/5 last:border-b-0 rounded-lg mx-1 my-1 hover:shadow-neon-sm ${config.modalResult}`}
                ...
                {/* Clusters */}
                {groupedResults.clusters.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-white font-bold text-sm mb-3">
                      {getSectionTitle('clusters')}
                    </h3>
                    {groupedResults.clusters.slice(0, 10).map((result) => (
                      <motion.button
                        key={result.id}
                        onClick={() => handleResultSelect(result)}
                        className={`flex items-center w-full text-left px-3 py-3 hover:bg-white/10 text-white transition-all duration-300 border-b border-white/5 last:border-b-0 rounded-lg mx-1 my-1 hover:shadow-neon-sm ${config.modalResult}`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-white/40 truncate mt-0.5">
                            {result.holder_addresses.length} wallet
                            {result.holder_addresses.length !== 1 ? 's' : ''}
                          </div>
                        </div>
                      </motion.button>
                    ))}
                ...
                {/* Nametags */}
                {groupedResults.nametags.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-white font-bold text-sm mb-3">
                      {getSectionTitle('nametags')}
                    </h3>
                    {groupedResults.nametags.slice(0, 10).map((result) => (
                      <motion.button
                        key={result.id}
                        onClick={() => handleResultSelect(result)}
                        className={`flex items-center w-full text-left px-3 py-3 hover:bg-white/10 text-white transition-all duration-300 border-b border-white/5 last:border-b-0 rounded-lg mx-1 my-1 hover:shadow-neon-sm ${config.modalResult}`}
