    ...
    return (
      <div className="bg-[#0A0A0A]/95 backdrop-blur-xl border border-[#FFFFFF20] p-3 rounded-2xl text-[#FFF] text-sm font-medium shadow-2xl transition-opacity duration-200">
        ...
        <p className="text-[#FFF] font-semibold">
          Price:{' '}
          <span className="text-emerald-400">{formatPrice(payload[0].value, currency, 8)}</span>
        </p>
      </div>
    )
  }
...
const MarketTab = ({ recaptchaRef, initialTokenSlug, onTokenSelect, toast, initialTokenData }) => {
  ...
  const getExplorerInfo = (chain, txHash, address) => {
    ...
    // Nếu có txHash và chain được hỗ trợ → dùng Explorer nội bộ
    ...
    // Fallback: dùng explorer bên ngoài
    if (normalizedChain === 'bitcoin') {
      return {
        url: txHash ? `https://mempool.space/tx/${txHash}` : '#',
    ...
    if (initialTokenSlug) {
      const fetchTokenBySlug = async () => {
        ...
        try {
          ...
          logger.error('Error fetching token by slug:', {
            slug: initialTokenSlug,
            error: err.message,
          })
        } finally {
  ...
  const renderPoolModalContent = () => {
    ...
    if (tokens.length < 2) {
      return (
        <p className="text-sm text-[#D4D4D4] text-center">Insufficient token data for this pool.</p>
      )
    }
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#FFF]">
                      {token1.holders?.count?.toLocaleString() || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">Top 10:</span>
                    <span className="text-[#FFF]">
                      {token1.holders?.distribution_percentage?.top_10 || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">11-30:</span>
                    <span className="text-[#FFF]">
                      {token1.holders?.distribution_percentage?.['11_30'] || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">31-50:</span>
                    <span className="text-[#FFF]">
                      {token1.holders?.distribution_percentage?.['31_50'] || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">Rest:</span>
                    <span className="text-[#FFF]">
                      {token1.holders?.distribution_percentage?.rest || 'N/A'}%
                    </span>
                  </div>
              ...
              <div className="bg-[#FFFFFF]/5 backdrop-blur-md rounded-xl p-4">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">Total Count:</span>
                    <span className="text-[#FFF]">
                      {token2.holders?.count?.toLocaleString() || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">Top 10:</span>
                    <span className="text-[#FFF]">
                      {token2.holders?.distribution_percentage?.top_10 || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">11-30:</span>
                    <span className="text-[#FFF]">
                      {token2.holders?.distribution_percentage?.['11_30'] || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">31-50:</span>
                    <span className="text-[#FFF]">
                      {token2.holders?.distribution_percentage?.['31_50'] || 'N/A'}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#D4D4D4]">Rest:</span>
                    <span className="text-[#FFF]">
                      {token2.holders?.distribution_percentage?.rest || 'N/A'}%
                    </span>
                  </div>
                  <div className="text-[#D4D4D4] text-xs pt-2 border-t border-[#FFFFFF10]">
                    Last Updated:{' '}
  ...
  useEffect(() => {
    if (selectedToken && timeRange && currency) {
      logger.log('Fetching price history:', {
      ...
      try {
        const percentageFieldMap = {
          0.5: {
            ...
            fallback: 'price_change_percentage_1h',
          },
          1: {
          ...
          7: {
            ...
            fallback: 'price_change_percentage_7d',
          },
          30: {
            currency: 'price_change_percentage_30d_in_currency',
            fallback: 'price_change_percentage_30d',
          },
          90: {
            currency: 'price_change_percentage_90d_in_currency',
            fallback: 'price_change_percentage_90d',
          },
          365: {
            currency: 'price_change_percentage_1y_in_currency',
            fallback: 'price_change_percentage_1y',
          },
        }
        ...
        const percentageChange =
          timeRange === '0.5'
            ? 'N/A'
            : (selectedToken[currencyField]?.[currency] ?? selectedToken[fallback] ?? 'N/A')
        const highLow = {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-row items-center justify-between gap-2 sm:gap-3">
            <div className="flex items-center gap-2 flex-shrink-0 bg-gradient-to-r from-[#FFFFFF]/10 to-[#FFFFFF]/5 backdrop-blur-md">
              <div className="p-1.5 group hover:scale-102 transition-transform duration-200">
                {' '}
                {/* CSS hover */}
                <h2 className="text-[8px] sm:text-[10px] font-bold text-[#FFF] uppercase tracking-wider">
                  Crypto
                </h2>
              </div>
            </div>
            ...
            <div className="flex flex-row items-center gap-4 flex-1 justify-end">
              ...
              <div className="relative" ref={chainDropdownRef}>
                <button
                  ...
                  className={`bg-[#0A0A0A]/40 backdrop-blur-md text-[#FFF] px-1.5 sm:px-2 py-1 sm:py-1 text-[8px] sm:text-[10px] flex items-center gap-1 sm:gap-2 border-2 border-[#FFFFFF20] hover:bg-[#FFFFFF]/10 transition-all duration-300 rounded-lg min-w-[120px] group hover:scale-102 active:scale-98 ${
                    selectedToken?.id &&
                ...
                >
                  ...
                  <span
                    className={`text-[8px] sm:text-[10px] ml-auto transition-transform duration-200 ${isChainDropdownOpen ? 'rotate-180' : ''}`}
                  >
                    {isChainDropdownOpen ? '▲' : '▼'}
                  </span>
                </button>
                {isChainDropdownOpen && (
                  ...
                  >
                    {getAvailableChains().length === 0 ? (
                      <div className="px-3 py-2 text-[#D4D4D4] text-[8px] sm:text-[10px]">
                  ...
                  {[...trendingTokens, ...trendingTokens].map((token, index) => (
                    ...
                    >
                      <Image
                        ...
                        unoptimized
                      />
                      <span className="text-[#FFF] text-[8px] sm:text-[10px] font-medium">
      ...
      {error && (
        <div className="text-[10px] sm:text-xs text-red-500 text-center p-2 sm:p-4 bg-red-500/10 border border-red-500/20 rounded-lg mb-2 animate-slideDown">
        ...
        >
          ...
          <div className="flex flex-col gap-4 max-h-full min-h-[800px] sm:max-h-full overflow-y-auto hide-scrollbar">
                ...
                <div className="relative">
                  <div className="absolute top-1 right-1 w-32 sm:w-40" ref={dropdownRef}>
                    ...
                    >
                      ...
                      <span
                        className={`ml-auto text-[10px] sm:text-xs transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`}
                      >
                        {isDropdownOpen ? '▲' : '▼'}
                      </span>
                    </button>
                            ...
                            .map((token) => (
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <div className="text-[8px] sm:text-[10px] text-[#D4D4D4]">
                                    {token.symbol?.toUpperCase() || 'Token'}
                                  </div>
                                </div>
                              </button>
                            ))}
                          {(searchQuery ? searchResults : tokens.slice(0, 30)).filter(isValidToken)
                            .length === 0 && (
                            <p className="text-[8px] sm:text-[10px] text-[#D4D4D4] text-center p-2">
                              No tokens found
                            </p>
                          )}
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="text-base sm:text-sm font-bold text-[#FFF] tracking-tight">
                          {selectedToken?.name ||
                            localCache.current[`token-metadata-${selectedToken?.id}`]?.data?.name}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-2 mb-2 sm:mb-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        >
                          {(selectedToken?.price_change_percentage_24h_in_currency?.[currency] ||
                            ...
                            ? `${
                                (selectedToken?.price_change_percentage_24h_in_currency?.[
                                  currency
                                ] ||
                                  localCache.current[`token-metadata-${selectedToken?.id}`]?.data
                                    ?.price_change_percentage_24h_in_currency?.[currency]) >= 0
                    ...
                    <div className="flex justify-end items-end gap-2">
                      {(selectedToken?.links?.twitter_screen_name ||
                        localCache.current[`token-metadata-${selectedToken?.id}`]?.data?.links
                          ?.twitter_screen_name) && (
                        <a
                            ...
                            className={`font-semibold ${
                              typeof (
                                ...
                                ? (selectedToken?.ath_change_percentage?.[currency] ||
                                    localCache.current[`token-metadata-${selectedToken?.id}`]?.data
                                      ?.ath_change_percentage?.[currency]) >= 0
                                  ? 'text-red-500'
                                  : 'text-emerald-400'
                                : 'text-[#FFF]'
                            }`}
                            ...
                            className={`font-semibold ${
                              typeof (
                                ...
                                ? (selectedToken?.atl_change_percentage?.[currency] ||
                                    localCache.current[`token-metadata-${selectedToken?.id}`]?.data
                                      ?.atl_change_percentage?.[currency]) >= 0
                                  ? 'text-red-500'
                                  : 'text-emerald-400'
                                : 'text-[#FFF]'
                            }`}
            ...
            >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center w-full max-w-[90%] sm:max-w-[600px] gap-2 sm:gap-3">
                  ...
                  <div className="flex items-center justify-center gap-2 sm:gap-4 mt-2 sm:mt-6 mb-2 sm:mb-0">
                    <div className="text-[8px] sm:text-[9px] text-[#FFF] text-center">
                      ...
                      <div
                        className={`font-bold ${
                          highLowData.percentageChange !== 'N/A' &&
                          typeof highLowData.percentageChange === 'number'
              ...
              {isChartLoading ? (
                <div className="h-48 sm:h-58 flex items-center justify-center"></div>
              ) : chartData && chartData.length > 0 ? (
                <div className="h-48 sm:h-58">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                      data={chartData}
                      margin={{ top: 10, right: 15, bottom: 0, left: isMobile ? 0 : 10 }}
                    >
              ...
              <div className="absolute top-1 right-1 flex items-center group p-2">
                <img
                  src="/logos/CG.webp"
                  alt="CG Logo"
                  className="w-4 sm:w-4 h-4 sm:h-4 object-contain opacity-60"
                />
                <span className="absolute right-20 sm:right-20 text-[8px] sm:text-[9px] text-[#D4D4D4] opacity-0 translate-x-4 group-hover:opacity-100 group-hover:-translate-x-0 transition-all duration-300 whitespace-nowrap flex items-center">
                  ...
                  <img
                    src="/logos/CG_1.webp"
                    alt="CG_1 Logo"
                    className="w-12 sm:w-12 h-12 sm:h-12 object-contain ml-2"
                  />
                </span>
              </div>
            </div>
          ...
          >
            {selectedToken ? (
              <>
                <div
                  className="flex w-full text-[10px] sm:text-[12px] bg-[#FFFFFF]/5 rounded-t-xl relative"
                  role="tablist"
                >
                  <button
                    onClick={() => {
                    ...
                    className={`flex-1 px-6 py-2 font-semibold transition-all duration-300 relative p-1 group ${
                      activeMarketTab === 'holders'
                        ? 'text-[#FFF]'
                        : 'text-[#D4D4D4] hover:text-[#FFF] hover:bg-[#FFFFFF]/5'
                    }`}
                    role="tab"
                ...
                <div className="flex-1 overflow-y-auto hide-scrollbar relative min-h-[500px] sm:min-h-[400px]">
                  ...
                  >
                    {activeMarketTab === 'holders' && (
                      <div
                        className="flex-1 tab-content relative min-h-[500px] sm:min-h-[400px]"
                        ref={autoAnimateRef}
                      >
                        {session ? (
                          <>
                            ...
                            {isLoadingOnChain ? (
                              ...
                              <div className="text-sm text-center p-6">
                                <p className="text-[#D4D4D4]">
                                  Unable to load top holders data. Please try again.
                                </p>
                              </div>
                                ...
                                <Virtuoso
                                  ...
                                  itemContent={(index, holder) => {
                                    ...
                                    const isValidAddress =
                                      holder.address &&
                                      (holder.address.match(/^0x[a-fA-F0-9]{40}$/) ||
                                        holder.address.match(/^(1|3|bc1)[a-zA-Z0-9]+$/))
                                    const HolderRow = React.memo(() => (
                                      <div className="flex border-t border-[#FFFFFF10] bg-[#0A0A0A]/80 px-3 py-2 text-[9px] sm:text-[11px]">
                                        <div className="flex-1 flex items-center gap-2 group relative">
                                          ...
                                          {isBitcoin && isValidAddress ? (
                                            ...
                                            <span
                                              ...
                                              onClick={() =>
                                                isValidAddress && handleAddressClick(holder.address)
                                              }
                                              title={holder.address}
                                            >
                                              <div className="flex flex-col">
                  ...
                  >
                    {activeMarketTab === 'cex' && (
                      <div
                        className="flex-1 overflow-x-auto overflow-y-auto tab-content custom-scrollbar hide-scrollbar relative min-h-[500px] sm:min-h-[400px]"
                        ref={autoAnimateRef}
                      >
                        <LoadingOverlay
                          isLoading={isLoadingTickers && !tickerData?.length}
                          isMobile={isMobile}
                          className="!absolute h-full w-full"
                        />
                        {tickerError ? (
                          <div className="text-[10px] sm:text-xs text-center p-6">
                            <p className="text-[#D4D4D4] mb-4">
                              Unable to load CEX markets data. Please try again.
                            </p>
                            <button
                          ...
                          <Virtuoso
                            ...
                            itemContent={(index, ticker) => {
                              const TickerRow = React.memo(() => (
                                <div className="flex border-t border-[#FFFFFF10] hover:bg-[#0A0A0A]/80 px-3 py-2 text-[9px] sm:text-[11px]">
                                  ...
                                  <div className="flex-1 text-center text-[#FFF] font-medium text-[9px] sm:text-[11px]">
                                    <span className="bg-[#FFFFFF]/5 px-1.5 py-0.5 rounded-md">
                  ...
                  >
                    {activeMarketTab === 'dex' && (
                      <>
                        ...
                        <div
                          className="flex-1 overflow-y-auto tab-content custom-scrollbar hide-scrollbar relative min-h-[500px] sm:min-h-[400px]"
                          ref={autoAnimateRef}
                        >
                          {session ? (
                            <>
                              <LoadingOverlay
                                isLoading={
                                  (selectedToken?.id === 'bitcoin'
                                    ? isLoadingMempool
                                    : isLoadingDex || isLoadingMoreDex || isLoadingPage) &&
                                  !(
                                    selectedToken?.id === 'bitcoin'
                                      ? mempoolTransactions
                                      : dexData.trades
                                ...
                                return trades.length > 0 ? (
                                          ...
                                          const TradeRow = React.memo(() => (
                                            <div className="flex border-t border-[#FFFFFF10] bg-[#0A0A0A]/80 py-1.5 px-2 text-[9px] sm:text-[11px]">
                                              ...
                                              <div className="flex-1 flex flex-col gap-1 items-center justify-center group relative">
                                                ...
                                                <span className="text-[7px] sm:text-[9px] text-[#D4D4D4] text-center">
                                              ...
                                              <div className="flex-[2] flex items-center justify-center gap-2 group relative">
                                                ...
                                                <button
                                                  ...
                                                  className="text-[#FFF] hover:text-[#FFF]/80 transition-colors font-medium text-[9px] sm:text-[11px] cursor-pointer"
                                                  title={
                                                    isBitcoin
                                                      ? item.inputs?.[0]?.address
                                                      : item.tx_from_address?.address
                                              ...
                                              <div className="flex-[2] flex items-center justify-center gap-2 group relative">
                                                ...
                                                <button
                                                  ...
                                                  className="text-[#FFF] hover:text-[#FFF]/80 transition-colors font-medium text-[10px] cursor-pointer"
                                                  title={
                                                    isBitcoin
                                                      ? item.outputs?.[0]?.address
                                                      : item.to_token_address?.address
                                              ...
                                              <div className="flex-1 flex flex-col gap-1 items-center justify-center text-[10px]">
                                                <span className="font-semibold flex items-center gap-2 text-[8px] sm:text-[10px]">
                                                  {isBitcoin ? (
                                                    <>
                                                      {(item.value_btc || 0).toLocaleString(
                                                        'en-US',
                                                        {
                                              ...
                                              <div className="flex-1 flex flex-col gap-1 items-center justify-center text-center text-[9px] sm:text-[11px]">
                                                {isBitcoin && (
                                                  <div className="text-[7px] sm:text-[9px] text-[#D4D4D4] text-center">
                                                    Fee: {item.fee.toLocaleString('en-US')} sat
                                                  </div>
                                                )}
                                                {isBitcoin ? (
                                                  <span
                                              ...
                                              {!isBitcoin && (
                                                <div className="flex-1 flex items-center justify-center">
                                                  <img
                                                    ...
                                                    onError={(e) => {
                                                      e.target.src = '/logos/ethereum.webp'
                                                    }}
                                                  />
                                                </div>
                                              )}
                                  ...
                                  !(isBitcoin
                                    ...
                                    <div className="text-[9px] sm:text-[11px] text-[#D4D4D4] text-center p-6">
                                      No {isBitcoin ? 'mempool transactions' : 'DEX data'} available
                                      for {selectedToken?.symbol?.toUpperCase() || 'selected token'}{' '}
                                      on{' '}
                                      {isBitcoin
                                        ? 'Bitcoin network'
            ...
            <div className="prose prose-invert max-w-none text-[#FFF] leading-relaxed">
              <ReactMarkdown
                ...
                components={{
                  ...
                  h2: ({ node, ...props }) => (
                    <h2 className="text-lg sm:text-xl font-semibold mt-3 mb-1" {...props} />
                  ),
                  p: ({ node, ...props }) => <p className="mb-2" {...props} />,
                  table: ({ node, ...props }) => (
                    <table
                      className="table-auto w-full border-collapse border border-[#FFFFFF20] my-2"
                      {...props}
                    />
                  ),
                  th: ({ node, ...props }) => (
