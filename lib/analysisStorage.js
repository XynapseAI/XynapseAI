...
export async function getAnalyzedWalletsWithTimestamps() {
    try {
        const result = await query(
            `SELECT wallet, last_analysis FROM wallet_analysis`
        );
        const walletsData = {};
        result.rows.forEach(row => {
            if (row.wallet && row.last_analysis) {
                walletsData[row.wallet.toLowerCase()] = row.last_analysis.toISOString();
            }
        });
        logger.info(`Fetched ${Object.keys(walletsData).length} analyzed wallet timestamps from PostgreSQL.`);
        return walletsData;
    } catch (e) {
        logger.error(`Error fetching analyzed wallets with timestamps from PostgreSQL: ${e.message}`);
        return {};
    }
}
