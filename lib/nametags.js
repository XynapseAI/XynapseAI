...
import { logger } from '../utils/serverLogger.js'; // Changed to named import
...
// Validate address (EVM or Bitcoin)
const isValidAddress = (addr) => isAddress(addr) || isBitcoinAddress(addr);
...
export async function getNametagsBatch(addresses) {
  const normalizedAddresses = addresses.map((addr) => addr.toLowerCase()).filter(isValidAddress);
  const foundNametags = {};
