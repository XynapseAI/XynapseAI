...
// Solana address validation (base58, 32-44 chars, no 0/O/I/l; case-sensitive)
const isSolanaAddress = (addr) => /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr);

// Validate address (EVM or Bitcoin or Solana; case-sensitive for Solana)
...
export async function getNametagsBatch(addresses) {
  // FIXED: Filter isValidAddress on original addresses (case-sensitive), then lower for DB
