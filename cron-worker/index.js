...
import { loadVaultSecrets } from '../utils/vault.js'; // Sử dụng vault.js từ utils
...
// Tải bí mật từ Vault và chạy cron job
(async () => {
  try {
    // Tải bí mật từ Vault
    await loadVaultSecrets();

    // Các biến môi trường từ Vault
    const ANALYZE_WALLETS_API_URL = process.env.ANALYZE_WALLETS_API_URL || (process.env.NEXTAUTH_URL || 'https://xynapseai.net') + '/api/analyze-wallets';
    ...
    if (!INTERNAL_API_TOKEN) {
      logger.error('INTERNAL_API_TOKEN is not defined in Vault');
      process.exit(1);
    }

    // Hàm sinh API key mới
    ...
    // Hàm tìm ví gửi tiền (deposit wallets)
    ...
    // Hàm lấy danh sách ví đang chờ xử lý
    ...
    async function runHighVolumeWalletAnalysis() {
      ...
      if (!ANALYZE_WALLETS_API_URL || ANALYZE_WALLETS_API_URL.includes('undefined')) {
        logger.error('Invalid ANALYZE_WALLETS_API_URL. Set NEXTAUTH_URL or ANALYZE_WALLETS_API_URL in Vault');
        return;
      }
    ...
    logger.error(`Failed to initialize cron job due to Vault error: ${vaultError.message}`, { stack: vaultError.stack });
    process.exit(1);
  }
})();