...
const pretrainedClassifierWeights = {
  layer1: [
    [-0.3852231502532959, 0.14592701196670532, -0.5980711579322815, 0.4895792603492737, 0.41286611557006836, -0.03675302863121033, 0.34242329001426697, 0.37972939014434814, 0.3775983154773712],
    [-0.06689132750034332, 0.33151912689208984, -0.542883038520813, -0.23388928174972534, -0.1634332686662674, -0.5747097134590149, 0.41045066714286804, -0.2539864480495453, -0.424101322889328],
    [-0.31863081455230713, 0.3511461317539215, 0.10715492069721222, -0.41582995653152466, -0.6143429279327393, -0.15023601055145264, 0.028768405318260193, -0.0374741330742836, -0.09149989485740662],
    [-0.18120448291301727, -0.18139006197452545, -0.6059948801994324, -0.42443862557411194, 0.035250164568424225, -0.3139837682247162, 0.5812512636184692, -0.23145431280136108, -0.25505200028419495],
    [-0.5485528111457825, -0.3241884112358093, -0.439699649810791, 0.09112757444381714, 0.4426422119140625, -0.3805481195449829, -0.17007115483283997, 0.22208809852600098, -0.27100348472595215],
    [0.23837096989154816, 0.4884408712387085, 0.11906836181879044, 0.2725473940372467, 0.29043710231781006, 0.49162957072257996, -0.31667372584342957, -0.03847460076212883, -0.4436405897140503],
    [-0.4314520061016083, 0.5624406337738037, -0.5410681962966919, 0.34240347146987915, 0.3620026707649231, -0.020901739597320557, -0.17321592569351196, -0.4140666723251343, -0.08774274587631226],
    [-0.45467469096183777, 0.405123770236969, -0.47632884979248047, -0.2866925299167633, -0.25550344586372375, -0.17906156182289124, 0.4458136558532715, 0.0271187424659729, 0.005169093608856201],
    [0.40937912464141846, -0.17941656708717346, 0.42937135696411133, -0.0015859007835388184, 0.18249744176864624, -0.4305436611175537, 0.04371905326843262, -0.5555511713027954, 0.12332719564437866]
  ],
  layer1_bias: [
...
async function fetchKnownNametags(addresses) {
  try {
    ...
    return new Map(known.map(k => [k.address.toLowerCase(), { label: k.nametag, image: k.image }]));
  } catch (err) {
...
function computeBehavioralFeatures(node, nodeTxs, knownExchanges) {
  ...
  node.airdropScore = (uniqueInSources.size > 10 && (incoming.length > 0 ? totalIn / incoming.length : 0) < 100) ? 1 : 0;

  // Interaction with exchanges: count connections to known exchanges
  node.interactionWithExchanges = [...uniqueInSources, ...new Set(outgoing.map(tx => tx.target.toLowerCase()))]
...
async function autoLabelWallets(wallets, tf = null, knownNametags) {
  ...
  for (const wallet of wallets) {
    ...
    if (maxSim > 0.8) { // Threshold for inheritance
...
async function detectClustersServer(nodes, edges, options = { useGNN: true, useDBSCAN: true, useIF: true }, tf = null, IsolationForest = null) {
  ...
  const knownExchanges = new Set([...knownNametags.keys()].filter(addr => knownNametags.get(addr)?.label?.includes('Exchange')));
  ...
  for (const [commId, group] of communityGroups) {
    ...
    if (group.wallets.some(w => w.interactionWithExchanges > 5)) cluster.behaviors.push('Exchange Interaction');
