...
function computeMetrics(transactions, wallets) {
  ...
  const times = allTxs.map(tx => getTime(tx.block_time)).filter(t => t !== null).sort((a, b) => a - b);
  let velocity = 0;
  if (times.length > 1) {
  ...
  const tokenKeys = allTxs.map(tx => tx.contractAddress?.toLowerCase() || tx.tokenSymbol?.toLowerCase() || 'unknown');
...
export function clusterInWorker(nodes, edges) {
  return new Promise((resolve, reject) => {
    const workerCode = `
      ...
      ${computePositionedNodes.toString()}
